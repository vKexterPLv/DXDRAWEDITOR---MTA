...
function colorPicker.create(id, start, title)
  ...
  cp.gui.inputHEX = guiCreateButton(16, cp.gui.height-60, cp.gui.width-32.4, 50, "Input hex color", false, cp.gui.window)
  ...
  addEventHandler("onClientGUIClick", cp.gui.inputHEX, cp.handlers.inputHEX, false)
  showCursor(true)
  return cp
end

function colorPicker:inputHEX()
	self:guiBlur()

	local function setValue(text)
		if getColorFromString(text) then
			self.color.h, self.color.s, self.color.v = rgb2hsv(getColorFromString(text))
			self:updateColor()
		end
	end
